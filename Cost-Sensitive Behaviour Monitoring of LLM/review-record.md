...
| The current failure analysis shows that high-quality aggregate output can hide degraded cases. | **Accept** | Several active-policy failures are `DEGRADED` cases accepted despite quality values above the low-quality threshold. | Add a bounded capability-coverage audit and then evaluate its selectivity against alternative probe schedules. | `results/five_failure_cases.csv`: cases 016–020 and 066–070 in the 120-case re-test. |
...
The current model is suitable as an **inspectable Week 1 simulation**, not as a calibrated probabilistic model. The revised implementation now has explicit cost-derived thresholds, corrected VOI accounting, and a bounded capability-coverage intervention. The next validation step is empirical calibration and probe-quality measurement.
