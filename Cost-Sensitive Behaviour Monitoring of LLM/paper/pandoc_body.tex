...
\subsection{3. Problem Formulation and Design Goals}\label{3-problem-formulation-and-design-goals}

Let the hidden state be

S = \{STABLE, DEGRADED, TRANSIENT, DISTRIBUTION\_SHIFT\}.
...
\tightlist
\item
  output quality below 0.70;
\item
  semantic drift above 0.30;
\item
  formatting failure;
\item
  safety shift;
\item
  latency above 900 ms;
\item
  API error rate above 0.08.
\end{enumerate}
...
\subsection{4. Probabilistic View}\label{4-probabilistic-view}

\subsubsection{4.1 Belief state}\label{41-belief-state}

The monitor maintains a probability distribution b(s) over the four hidden states. Before observing a case, the belief is the hypothetical prior above. After an observation x, Bayes\textquotesingle{} rule gives

P(S=s \textbar{} x) = P(S=s) P(x \textbar{} S=s) / sum\_s\textquotesingle{} P(S=s\textquotesingle) P(x \textbar{} S=s\textquotesingle).

The important revision is the form of P(x \textbar{} s).

\subsubsection{4.2 Joint initial evidence model}\label{42-joint-initial-evidence-model}

The earlier prototype approximated the joint evidence probability as a product of marginal probabilities:

P(x \textbar{} s) approximately equal to product over i of P(E\_i \textbar{} s).

That assumption is not justified when signals co-vary. To address it, the revised simulator introduces a shared latent failure-pressure variable that perturbs quality, semantic drift, formatting probability, latency, and error rate together. The average absolute pairwise correlation among the six binary evidence indicators in 6,000 training cases is 0.265 for STABLE and 0.319 for DEGRADED, with maximum pairwise absolute correlations of 0.728 and 0.936 respectively. Several binary channels are constant within TRANSIENT or DISTRIBUTION\_SHIFT in this simulator, so some within-state pairwise correlations are undefined rather than evidence of independence.
...
P(z \textbar{} s) = (N(z,s) + alpha) / (N(s) + 64 alpha),
...
\subsubsection{4.3 Worked posterior example}\label{43-worked-posterior-example}
...
\textbar{} State \textbar{} P(signature \textbar{} state) \textbar{} \textbar-\/-\/-\textbar-\/-\/-:\textbar{} \textbar{} STABLE \textbar{} 0.000303 \textbar{} \textbar{} DEGRADED \textbar{} 0.010116 \textbar{} \textbar{} TRANSIENT \textbar{} 0.001106 \textbar{} \textbar{} DISTRIBUTION\_SHIFT \textbar{} 0.093373 \textbar{}
...
\subsection{5. Information Theory}\label{5-information-theory}

\subsubsection{5.1 Entropy}\label{51-entropy}

For belief vector b,

H(b) = - sum\_s P(s) log2 P(s).
...
\subsubsection{5.2 Expected information gain}\label{52-expected-information-gain}

For a binary probe X,

EIG(X) = H(B) - P(X=1) H(B \textbar{} X=1) - P(X=0) H(B \textbar{} X=0).
...
\bottomrule\noalign{}
\endlastfoot
capability\_issue & 0.1452 & 0.4099 \\
error\_high & 0.1888 & 0.3380 \\
latency\_high & 0.0717 & 0.3169 \\
semantic\_drift & 0.1836 & 0.2043 \\
format\_failure & 0.1071 & 0.0177 \\
safety\_shift & 0.0094 & 0.0057 \\
\end{longtable}

These values are calculated from the learned synthetic observation model. They are not production information rates.

\subsubsection{5.3 Value of information versus information gain}\label{53-value-of-information-versus-information-gain}

The monitor also computes decision-aware value of information. Let C\_direct(b) be the minimum expected cost of ACCEPT or REJECT under belief b. For a binary probe q,

GrossVOI(q) = C\_direct(b) - {[}P(q=1) C\_direct(b\_q1) + P(q=0) C\_direct(b\_q0){]}.

NetVOI(q) = GrossVOI(q) - C\_probe.
...
At the prior with investigation cost 0.4, the direct state-aware cost is 2.000. The one-step expected costs after candidate probes are 1.194 for capability\_issue, 1.565 for error\_high, 1.622 for semantic\_drift, 2.189 for latency\_high, 2.227 for format\_failure, and 2.367 for safety\_shift. The corresponding gross value for capability\_issue is 1.206, so the probe clears the 0.4 cost threshold.

\subsubsection{5.4 Calibration metric}\label{54-calibration-metric}

We measure Brier error for the probability of any non-STABLE state:

Brier = (1/N) sum\_i (p\_i - y\_i)\^{}2.
...
\subsection{6. Evidence Acquisition}\label{6-evidence-acquisition}

\subsubsection{6.1 Probe set}\label{61-probe-set}
...
The capability probe was introduced because failure analysis from the previous design exposed cases where aggregate quality remained healthy while a narrow capability was degraded. Its synthetic likelihood is 0.01 in STABLE, 0.95 in DEGRADED, 0.10 in TRANSIENT, and 0.20 in DISTRIBUTION\_SHIFT.

\subsubsection{6.2 Acquisition policies}\label{62-acquisition-policies}

The revised evaluation compares four acquisition mechanisms.

\textbf{Random-acquire} chooses one probe uniformly at random and always pays its cost.

\textbf{Always-acquire} always obtains the targeted capability probe and then acts.

\textbf{EIG one-step} chooses the probe with maximum expected entropy reduction, obtains one probe, then acts.

\textbf{VOI one-step} chooses the probe with minimum expected total decision cost over one remaining probe.

\textbf{VOI two-step}, the main policy, plans over at most two probes. It recursively compares the direct action with each admissible probe and evaluates the best downstream decision or second probe after each possible outcome.
...
\subsection{7. Decision Policy}\label{7-decision-policy}

\subsubsection{7.1 Four-state cost model}\label{71-four-state-cost-model}
...
\subsubsection{7.2 Two-step lookahead}\label{72-two-step-lookahead}

Let V\_k(b) be the minimum expected cost with k probes remaining. Then

V\_0(b) = min\_a C(a \textbar{} b).

For k \textgreater{} 0,

V\_k(b) = min( V\_0(b), min\_q {[} C\_probe + P(q=1)V\_\{k-1\}(b\_q1) + P(q=0)V\_\{k-1\}(b\_q0) {]} ).
...
\subsubsection{7.3 Stopping rule}\label{73-stopping-rule}
...
\subsection{8. Experimental Design}\label{8-experimental-design}

\subsubsection{8.1 Synthetic data generator}\label{81-synthetic-data-generator}

Each evaluation case is assigned one of the four hidden states. The 50-case scenario contains 11 DEGRADED cases, 7 TRANSIENT cases, 5 DISTRIBUTION\_SHIFT cases, and 27 STABLE cases. The 120-case generator repeats this state schedule cyclically while independently sampling observations.
...
\subsubsection{8.2 Training, development, and test separation}\label{82-training-development-and-test-separation}
...
\subsubsection{8.3 Baselines and metrics}\label{83-baselines-and-metrics}
...
\item
  legacy binary naive-Bayes decision;
\item
  binary decision with the revised joint evidence model;
\item
  four-state direct expected-cost decision without acquisition;
\item
  random acquisition;
\item
  always-acquire;
\item
  one-step entropy-greedy acquisition;
\item
  one-step decision-aware VOI;
\item
  two-step decision-aware VOI.
\end{enumerate}

Metrics are false ACCEPTs, false REJECTs, number of investigations, investigation rate, decision cost, event recall, event precision, and Brier error. Event recall treats any non-STABLE state as a detection target. State-specific interpretation is also retained because a single event metric can hide differences among DEGRADED, TRANSIENT, and DISTRIBUTION\_SHIFT.

\subsection{9. Results}\label{9-results}

\subsubsection{9.1 Seed-7 diagnostic runs}\label{91-seed-7-diagnostic-runs}
...
\subsubsection{9.3 Twenty-seed held-out evaluation}\label{93-twenty-seed-held-out-evaluation}
...
\subsubsection{9.4 Calibration and correlated evidence}\label{94-calibration-and-correlated-evidence}
...
\subsubsection{9.5 Investigation-cost sensitivity}\label{95-investigation-cost-sensitivity}
...
\subsection{10. Failure Analysis}\label{10-failure-analysis}
...
\bottomrule\noalign{}
\endlastfoot
Case & case-037, seed 7 & DISTRIBUTION\_SHIFT under-detection & Quality 0.714 and semantic drift 0.281; capability probe negative & ACCEPT, true cost 6.4 \\
...
Case & case-003, seed 7 & Additional acquisition on a stable case & Two-step path acquires semantic\_drift then capability\_issue & ACCEPT, probe cost 0.8 \\
...
\subsubsection{10.1 State-level behavior}\label{101-state-level-behavior}

On the regenerated 120-case seed-7 active run, the benchmark contains 69 STABLE, 27 DEGRADED, 14 TRANSIENT, and 10 DISTRIBUTION\_SHIFT cases. The active policy rejects all 27 DEGRADED and all 14 TRANSIENT cases, rejects 9 of 10 DISTRIBUTION\_SHIFT cases, and falsely rejects 2 of 69 STABLE cases. The remaining accepted non-STABLE case is a distribution shift. Event recall is therefore 50/51 = 0.980 and event precision is 50/52 = 0.962.
...
Third, the decision objective matters. The binary policies price only DEGRADED versus not-DEGRADED and perform substantially worse in decision cost. The state-aware policy explicitly prices TRANSIENT and DISTRIBUTION\_SHIFT. This makes the mapping from latent states to operational consequences inspectable and avoids treating all non-DEGRADED states as equivalent.

The revised design is directly related to active feature acquisition and acquisition-POMDP work, which formalize the trade-off between the information value of a measurement and its acquisition cost. It is also complementary to change-point detection: the present monitor decides what to do with a case after evidence is observed, whereas sequence-level change-point methods detect when a stream has changed. A future version should combine these layers rather than treating DISTRIBUTION\_SHIFT as an isolated case label.

\subsection{12. Limitations and Knowledge Gaps}\label{12-limitations-and-knowledge-gaps}

Several limitations remain unresolved.

\textbf{Synthetic evidence and priors.} All priors, conditional observation mechanisms, and state costs are synthetic. They are useful for controlled evaluation but cannot establish production reliability.

\textbf{Human labels and evaluator reliability.} The benchmark has programmatic state labels. It does not include human-reviewed LLM outputs, inter-rater disagreement, judge calibration, or label drift. Work on evaluator reliability and checklist-based judging motivates these concerns, but the present benchmark does not measure them.

\textbf{Sequential probe dependence.} The six initial channels use a full joint distribution, but repeated probes remain conditionally independent given state. A future model should learn joint distributions over probe sequences or explicitly represent temporal dynamics.

\textbf{Probe latency and monetary cost.} Investigation cost is normalized. We do not currently measure API latency, human review time, or monetary spend for individual probes. The 0.4 value is selected from a development split and should not be interpreted as an empirical currency value.

\textbf{Real drift and change points.} DISTRIBUTION\_SHIFT is a generated state, not a production change point detector. Classical change-point methods remain relevant for sequence-level monitoring {[}5{]}. The current work treats a case as the unit of decision and does not infer change-point boundaries over a long stream.

\textbf{Learned acquisition policies.} The paper compares transparent random, always-acquire, EIG, one-step VOI, and two-step VOI strategies. It does not implement a reinforcement-learning acquisition policy. A learned policy trained only on this simulator could simply learn simulator artifacts, so adding it without real traces would not resolve the external-validity problem.

\textbf{Cost cliff.} The sensitivity table shows that a hypothetical investigation cost near 0.8 sharply changes behavior. This is not hidden. It is evidence that cost calibration is itself a major design requirement.

\textbf{Production governance.} The monitor is not an autonomous incident commander. High-impact operational actions should remain subject to human review and deployment-specific policy.
...
{[}1{]} Lingjiao Chen, Matei Zaharia, and James Zou. How is ChatGPT\textquotesingle s behavior changing over time? arXiv:2307.09009, 2023.
